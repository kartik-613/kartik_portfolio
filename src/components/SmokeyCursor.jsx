...
export default function SmokeyCursor({
    ...
    transparent = true,
    className = "",
    // props below are defined but not used in the original calculation logic
    // keeping them in the signature if the user wants to extend later,
    // but commenting them to avoid lint errors if necessary.
    // disabled = false,
    // intensity = 1,
    // followMouse = true,
    // autoColors = true,
    ...
    `
        ...
        return () => {
            ...
            document.body.removeEventListener("mousemove", handleFirstMouseMove);
            document.body.removeEventListener("touchstart", handleFirstTouchStart);
            // Ensure any pending animation frame is canceled if needed
            // (though updateFrame is self-limiting by config.PAUSED)
            config.PAUSED = true;
        }
    }, [
    ...
    return (
        <div className={`fixed top-0 left-0 z-[9999] pointer-events-none w-full h-full ${className}`}>
            <canvas
                ref={canvasRef}
                id="fluid"
